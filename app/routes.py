...
def components_list():
    ...
    return render_template(
        "components_list.html", title="Components", components=components
    )
...
def add_new_component():
    ...
    return render_template(
        "add_new_component.html", title="Add new Component", form=form
    )
...
def add_component_comment(component_id):
    ...
    return render_template(
        "add_component_comment.html", title=f"{component.component_name}", form=form
    )
...
def soi_list():
    ...
    return render_template("soi_list.html", title="SOI", sois=sois)


@app.route("/soi_view/<soi_id>", methods=["GET", "POST"])
def soi_view(soi_id):
    soi = SOI.query.get(soi_id)
    return render_template("soi_view.html", title=f"{soi.soi_name}", soi=soi)
...
def add_new_soi():
    ...
    return render_template("add_new_soi.html", title="Add new SOI", form=form)


@app.route("/systems_list")
def systems_list():
    systems = System.query.order_by(System.system_name.desc())
    return render_template("systems_list.html", title="Systems", systems=systems)
...
def add_new_system():
    ...
    return render_template("add_new_system.html", title="Add new system", form=form)
